...
//#region node_modules/osmtogeojson/lodash.custom.js
var require_lodash_custom = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	(function() {
		...
		*/
		function baseMerge(object, source, srcIndex, customizer, stack) {
			...
			if (!(isArray(source) || isTypedArray(source))) var props = baseKeysIn(source);
		...
		*/
		function baseMergeDeep(object, source, key, srcIndex, mergeFunc, customizer, stack) {
			...
			if (isCommon) {
				newValue = srcValue;
				if (isArray(srcValue) || isTypedArray(srcValue)) if (isArray(objValue)) newValue = objValue;
		...
		*/
		function isEmpty(value) {
			if (isArrayLike(value) && (isArray(value) || typeof value == "string" || typeof value.splice == "function" || isBuffer(value) || isArguments(value))) return !value.length;
...
//#endregion
//#region node_modules/@mapbox/geojson-rewind/index.js
var require_geojson_rewind = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	module.exports = rewind;
	function rewind(gj, outer) {
		var type = gj && gj.type, i;
		if (type === "FeatureCollection") for (i = 0; i < gj.features.length; i++) rewind(gj.features[i], outer);
		else if (type === "GeometryCollection") for (i = 0; i < gj.geometries.length; i++) rewind(gj.geometries[i], outer);
		else if (type === "Feature") rewind(gj.geometry, outer);
		else if (type === "Polygon") rewindRings(gj.coordinates, outer);
		else if (type === "MultiPolygon") for (i = 0; i < gj.coordinates.length; i++) rewindRings(gj.coordinates[i], outer);
		return gj;
	}
	function rewindRings(rings, outer) {
		if (rings.length === 0) return;
		rewindRing(rings[0], outer);
		for (var i = 1; i < rings.length; i++) rewindRing(rings[i], !outer);
	}
...
//#endregion
//#region node_modules/osm-polygon-features/polygon-features.json
var require_polygon_features = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	module.exports = [
		{
			"key": "building",
			"polygon": "all"
...
//#endregion
//#region node_modules/osm-polygon-features/index.js
var require_osm_polygon_features = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	module.exports = require_polygon_features();
}));
//#endregion
//#region node_modules/osmtogeojson/index.js
var require_osmtogeojson = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	function default_deduplicator(objectA, objectB) {
		if ((objectA.version || objectB.version) && objectA.version !== objectB.version) return (+objectA.version || 0) > (+objectB.version || 0) ? objectA : objectB;
	...
	osmtogeojson = function(data, options, featureCallback) {
		...
		var result;
		if (typeof XMLDocument !== "undefined" && data instanceof XMLDocument || typeof XMLDocument === "undefined" && data.childNodes) result = _osmXML2geoJSON(data);
		...
		function _overpassJSON2geoJSON(json) {
			...
			function fullGeometryRelation(rel) {
				...
				function addFullGeometryWay(geometry, id) {
					...
					geometry.forEach(function(nd) {
						if (nd) addFullGeometryWayPseudoNode(nd.lat, nd.lon);
						else geometryWay.nodes.push(void 0);
					});
					ways.push(geometryWay);
				}
				rel.members.forEach(function(member, i) {
					if (member.type == "node") {
						if (member.lat) addFullGeometryNode(member.lat, member.lon, member.ref);
			...
			for (var i = 0; i < json.elements.length; i++) switch (json.elements[i].type) {
				case "node":
		...
		function _osmXML2geoJSON(xml) {
			...
			function fullGeometryWay(way, nds) {
				...
				_.each(nds, function(nd, i) {
					if (nd.getAttribute("lat")) addFullGeometryNode(nd.getAttribute("lat"), nd.getAttribute("lon"), way.nodes[i]);
				});
			}
			function fullGeometryRelation(rel, members) {
				...
				function addFullGeometryWay(nds, id) {
					...
					_.each(nds, function(nd) {
						if (nd.getAttribute("lat")) addFullGeometryWayPseudoNode(nd.getAttribute("lat"), nd.getAttribute("lon"));
						else geometryWay.nodes.push(void 0);
					});
					ways.push(geometryWay);
				}
				_.each(members, function(member, i) {
					if (rel.members[i].type == "node") {
						if (member.getAttribute("lat")) addFullGeometryNode(member.getAttribute("lat"), member.getAttribute("lon"), rel.members[i].ref);
			...
			_.each(xml.getElementsByTagName("way"), function(way, i) {
				...
				if (has_full_geometry) fullGeometryWay(wayObject, way.getElementsByTagName("nd"));
				else if (bounds = way.getElementsByTagName("bounds")[0]) boundsGeometry(wayObject, bounds);
				ways.push(wayObject);
			});
			_.each(xml.getElementsByTagName("relation"), function(relation, i) {
				...
				if (has_full_geometry) fullGeometryRelation(relObject, relation.getElementsByTagName("member"));
				else if (bounds = relation.getElementsByTagName("bounds")[0]) boundsGeometry(relObject, bounds);
				rels.push(relObject);
			});
			return _convert2geoJSON(nodes, ways, rels);
		}
		function _convert2geoJSON(nodes, ways, rels) {
			...
			var nodeids = /* @__PURE__ */ new Object();
			var poinids = /* @__PURE__ */ new Object();
			for (var i = 0; i < nodes.length; i++) {
				var node = nodes[i];
				if (nodeids[node.id] !== void 0) node = options.deduplicator(node, nodeids[node.id]);
				nodeids[node.id] = node;
				if (typeof node.tags != "undefined" && has_interesting_tags(node.tags)) poinids[node.id] = true;
			}
			for (var i = 0; i < rels.length; i++) if (_.isArray(rels[i].members)) {
				for (var j = 0; j < rels[i].members.length; j++) if (rels[i].members[j].type == "node") poinids[rels[i].members[j].ref] = true;
			}
			var wayids = /* @__PURE__ */ new Object();
			var waynids = /* @__PURE__ */ new Object();
			for (var i = 0; i < ways.length; i++) {
				var way = ways[i];
				if (wayids[way.id]) way = options.deduplicator(way, wayids[way.id]);
				wayids[way.id] = way;
				if (_.isArray(way.nodes)) for (var j = 0; j < way.nodes.length; j++) {
			...
			for (var i = 0; i < rels.length; i++) {
				var rel = rels[i];
				if (relids[rel.id]) rel = options.deduplicator(rel, relids[rel.id]);
				relids[rel.id] = rel;
			}
			var relsmap = {
				node: {},
			...
			for (var id in relids) {
				...
				for (var j = 0; j < rel.members.length; j++) {
					...
					if (typeof m_ref !== "number") m_ref = m_ref.replace("_fullGeom", "");
					if (!relsmap[m_type]) {
			...
			var geojsonpolygons = [];
			for (var i = 0; i < rels.length; i++) {
				if (relids[rels[i].id] !== rels[i]) continue;
				if (typeof rels[i].tags != "undefined" && (rels[i].tags["type"] == "route" || rels[i].tags["type"] == "waterway")) {
					...
					function construct_multilinestring(rel) {
						...
						var members = rel.members.filter(function(m) {
							return m.type === "way";
						});
						members = members.map(function(m) {
							var way = wayids[m.ref];
							if (way === void 0 || way.nodes === void 0) {
				...
				if (typeof rels[i].tags != "undefined" && (rels[i].tags["type"] == "multipolygon" || rels[i].tags["type"] == "boundary")) {
					...
					var outer_count = 0;
					for (var j = 0; j < rels[i].members.length; j++) if (rels[i].members[j].role == "outer") outer_count++;
					else if (options.verbose && rels[i].members[j].role != "inner") console.warn("Multipolygon", rels[i].type + "/" + rels[i].id, "member", rels[i].members[j].type + "/" + rels[i].members[j].ref, "ignored because it has an invalid role: \"" + rels[i].members[j].role + "\"");
					rels[i].members.forEach(function(m) {
						if (wayids[m.ref]) {
					...
					var simple_mp = false;
					if (outer_count == 1 && !has_interesting_tags(rels[i].tags, { "type": true })) simple_mp = true;
					var feature = null;
					if (!simple_mp) feature = construct_multipolygon(rels[i], rels[i]);
					else {
					...
					function construct_multipolygon(tag_object, rel) {
						...
						members = _.compact(members);
						var outers = join(members.filter(function(m) {
							return m.role === "outer";
						...
						for (var j = 0; j < inners.length; j++) {
							...
							if (o !== void 0) mp[o].push(inners[j]);
							else if (options.verbose) console.warn("Multipolygon", mp_geometry + "/" + mp_id, "contains an inner ring with no containing outer");
						}
						var mp_coords = [];
			...
			for (var i = 0; i < ways.length; i++) {
				if (wayids[ways[i].id] !== ways[i]) continue;
				...
				if (typeof ways[i].id !== "number") ways[i].id = +ways[i].id.replace("_fullGeom", "");
				ways[i].tainted = false;
				ways[i].hidden = false;
				var coords = new Array();
				for (j = 0; j < ways[i].nodes.length; j++) if (typeof ways[i].nodes[j] == "object") coords.push([+ways[i].nodes[j].lon, +ways[i].nodes[j].lat]);
				else {
				...
				if (!featureCallback) if (way_type == "LineString") geojsonlines.push(feature);
			...
			if (options.flatProperties) geojson.features.forEach(function(f) {
